...
- **RetroAchievements Integration.** Final testing with `RA_API_KEY`. Verify definition fetch, icon loading, and user account linking.

- **Improved ROM Import UX.** Better guidance and auto-suggestions when importing ROM files that don't match default executable patterns.

## Planned / Wishlist

### High Priority

- Achievement toast overlays / in-game popups (Windows transparent window or Goldberg built-in overlay + Steam Deck Decky Loader plugin)
- Goldberg client-side DLL injection & monitoring (copy DLL before launch, read `achievements.json`, report unlocks)
