    ...
    <div class="flex-1 overflow-y-auto px-8 py-6">
      ...
      >
        ...
        <div class="bg-zinc-900/50 rounded-xl p-4 space-y-3">
          <div class="flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-4 text-amber-400">
              <path fill-rule="evenodd" d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd" />
            </svg>
            <p class="font-medium text-zinc-200 text-sm">Save Conflict Dialog</p>
          </div>
          <p class="text-zinc-500 text-xs">
            Emits a fake conflict event. Navigate to a game page first, then come back and trigger.
          </p>
          <div class="flex items-center gap-2">
            <input
              v-model="debugConflictGameId"
              type="text"
              placeholder="Game ID"
              class="flex-1 rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs text-zinc-100 placeholder-zinc-600 focus:border-blue-500 focus:outline-none font-mono"
            />
            <button
              :ref="(el: any) => registerContent(el, { onSelect: emitFakeConflict })"
              class="px-3 py-2 rounded-lg text-xs font-medium bg-amber-600/20 text-amber-400 border border-amber-500/50 hover:bg-amber-600/30 transition-colors whitespace-nowrap"
              @click="emitFakeConflict"
            >
              Emit
            </button>
          </div>
          <p v-if="debugConflictEmitted" class="text-xs text-green-400">
            Emitted! Go to the game page to see the dialog.
          </p>
        </div>
        ...
        <div class="bg-zinc-900/50 rounded-xl p-4 space-y-3">
          <div class="flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-4 text-purple-400">
              <path d="M16.364 3.636a.75.75 0 00-1.06 1.06 7.5 7.5 0 010 10.607.75.75 0 001.06 1.061 9 9 0 000-12.728zM4.697 4.697a.75.75 0 00-1.061-1.06 9 9 0 000 12.727.75.75 0 101.06-1.06 7.5 7.5 0 010-10.607zM12.475 7.525a.75.75 0 00-1.06 1.06 2.5 2.5 0 010 3.536.75.75 0 001.06 1.06 4 4 0 000-5.656zM8.586 8.586a.75.75 0 00-1.061-1.061 4 4 0 000 5.656.75.75 0 001.06-1.06 2.5 2.5 0 010-3.536zM10 11a1 1 0 100-2 1 1 0 000 2z" />
            </svg>
            <p class="font-medium text-zinc-200 text-sm">Sunshine Streaming</p>
          </div>

          <!-- Admin credentials for starting Sunshine -->
          <div
            v-if="debugSunshineStatus?.installed && !debugSunshineStatus?.running"
            class="flex items-center gap-2"
          >
            <input
              v-model="debugSunshineAdminUser"
              type="text"
              placeholder="Admin username"
              class="flex-1 rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs text-zinc-100 placeholder-zinc-600 focus:border-blue-500 focus:outline-none font-mono"
            />
            <input
              v-model="debugSunshineAdminPass"
              type="password"
          ...
          <div class="flex items-center gap-2 flex-wrap">
            <button
              :ref="(el: any) => registerContent(el, { onSelect: doCheckSunshine })"
              class="px-3 py-2 rounded-lg text-xs font-medium bg-zinc-800 text-zinc-300 border border-zinc-700 hover:bg-zinc-700 transition-colors"
              :disabled="debugStreamingLoading"
              @click="doCheckSunshine"
            >
              Check Status
            </button>
            <button
              v-if="debugSunshineStatus && !debugSunshineStatus.installed"
              :ref="(el: any) => registerContent(el, { onSelect: doInstallSunshine })"
              class="px-3 py-2 rounded-lg text-xs font-medium bg-blue-600/20 text-blue-400 border border-blue-500/50 hover:bg-blue-600/30 transition-colors"
              :disabled="debugStreamingLoading"
            ...
            <button
              v-if="debugSunshineStatus?.installed && !debugSunshineStatus?.running"
              :ref="(el: any) => registerContent(el, { onSelect: doStartSunshine })"
              class="px-3 py-2 rounded-lg text-xs font-medium bg-green-600/20 text-green-400 border border-green-500/50 hover:bg-green-600/30 transition-colors"
              :disabled="debugStreamingLoading"
            ...
            <button
              v-if="debugSunshineStatus?.running"
              :ref="(el: any) => registerContent(el, { onSelect: doStopSunshine })"
              class="px-3 py-2 rounded-lg text-xs font-medium bg-red-600/20 text-red-400 border border-red-500/50 hover:bg-red-600/30 transition-colors"
              :disabled="debugStreamingLoading"
            ...
            <button
              :ref="(el: any) => registerContent(el, { onSelect: doFetchSessions })"
              class="px-3 py-2 rounded-lg text-xs font-medium bg-zinc-800 text-zinc-300 border border-zinc-700 hover:bg-zinc-700 transition-colors"
            ...
            <button
              :ref="(el: any) => registerContent(el, { onSelect: doCreateTestSession })"
              class="px-3 py-2 rounded-lg text-xs font-medium bg-purple-600/20 text-purple-400 border border-purple-500/50 hover:bg-purple-600/30 transition-colors"
              @click="doCreateTestSession"
            >
              Create Session
            </button>
            <button
              v-if="debugStreamingSessions.length > 0"
              :ref="(el: any) => registerContent(el, { onSelect: doStopAllSessions })"
              class="px-3 py-2 rounded-lg text-xs font-medium bg-red-600/20 text-red-400 border border-red-500/50 hover:bg-red-600/30 transition-colors"
          ...
          <div
            v-if="debugSunshineStatus"
            class="rounded-lg bg-zinc-950/50 p-3 text-xs font-mono text-zinc-400 space-y-0.5"
          >
            <div>installed: <span :class="debugSunshineStatus.installed ? 'text-green-400' : 'text-red-400'">{{ debugSunshineStatus.installed }}</span></div>
            <div>running: <span :class="debugSunshineStatus.running ? 'text-green-400' : 'text-zinc-600'">{{ debugSunshineStatus.running }}</span></div>
            <div v-if="debugSunshineStatus.version">version: {{ debugSunshineStatus.version }}</div>
            <div v-if="debugSunshineStatus.web_ui_url">web_ui: {{ debugSunshineStatus.web_ui_url }}</div>
            <div>paired: {{ debugSunshineStatus.paired_clients }}</div>
          </div>

          <div
            v-if="debugStreamingSessions.length > 0"
            class="rounded-lg bg-zinc-950/50 p-3 text-xs font-mono text-zinc-400 max-h-32 overflow-y-auto space-y-2"
          >
            <div v-for="s in debugStreamingSessions" :key="s.id" class="pb-2 border-b border-zinc-800 last:border-0">
              <div>{{ s.game?.mName ?? 'Desktop' }} — <span :class="s.status === 'Ready' ? 'text-green-400' : 'text-yellow-400'">{{ s.status }}</span></div>
              <div>host: {{ s.hostClient.name }} | ip: {{ s.hostLocalIp ?? '?' }}</div>
            </div>
          </div>
          <div v-else-if="debugSessionsFetched" class="text-xs text-zinc-600">
            No active sessions.
          </div>

          <p v-if="debugStreamingError" class="text-xs text-red-400">{{ debugStreamingError }}</p>
        </div>
      </div>
...
const debugConflictGameId = ref("");
const debugConflictEmitted = ref(false);

async function emitFakeConflict() {
  if (!debugConflictGameId.value) return;

  const fakeConflicts = {
    gameId: debugConflictGameId.value,
    conflicts: [
      {
  ...
  await emit(
    `save_sync_conflict/${debugConflictGameId.value}`,
    fakeConflicts,
  );

  debugConflictEmitted.value = true;
  setTimeout(() => {
    debugConflictEmitted.value = false;
  }, 5000);
}

const {
  sunshineStatus: debugSunshineStatus,
  loading: debugStreamingLoading,
  error: debugStreamingError,
  checkSunshine,
  installSunshine,
  startSunshine,
  stopSunshine,
...
const debugStreamingSessions = ref<StreamingSession[]>([]);
const debugSessionsFetched = ref(false);
const debugSunshineAdminUser = ref("sunshine");
const debugSunshineAdminPass = ref("");
...
async function doStartSunshine() {
  try {
    await startSunshine(debugSunshineAdminUser.value, debugSunshineAdminPass.value);
  } catch {
    // error shown in template
  }
}
...
async function doFetchSessions() {
  debugStreamingSessions.value = await listRemoteSessions();
  debugSessionsFetched.value = true;
}
...
async function doCreateTestSession() {
  try {
    ...
    console.error("[BPM:DEBUG] Failed to create test session:", msg);
    debugStreamingError.value = msg;
  }
}

async function doStopAllSessions() {
  try {
    for (const s of debugStreamingSessions.value) {
      await stopStreamingSession(s.id);
    }
    await doFetchSessions();
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.error("[BPM:DEBUG] Failed to stop sessions:", msg);
    debugStreamingError.value = msg;
  }
}
</script>
